...
const AdminSettings = () => {
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const settingsData = response.data as { settings?: any[] };
        ...
        // اگر تنظیماتی وجود نداشت، مقادیر پیش‌فرض را بارگذاری می‌کنیم
  ...
  const handleSave = async (category: string) => {
    setSaving(true);
    const toastId = toast.loading(`در حال ذخیره تنظیمات ${getCategoryName(category)}...`);
    
    try {
      const categorySettings = Object.values(settings)
      ...
      const response = await adminApi.bulkUpdateSettings(categorySettings);
